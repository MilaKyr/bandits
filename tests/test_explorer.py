...
def explorer(data_info):
    return Explorer(data_info, n_features=2, test_size=0.5, seed=1234, save_experiment=False)
...
def test_explorer_experiment(explorer, mocker):
    ...
    history_stats_result, regret_stats_result = explorer.experiment([bandit], n_rounds=1)
    regret_stats_expected_result = {bandit.name(): [4.]}
    history_stats_expected_result = {bandit.name(): [[0., 0., 0., 1., 0.]]}
