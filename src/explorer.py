...
class DatasetHelper:
    ...
    def get_data(
        self, data_info: DataInformation, test_size: float, shuffle: bool
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        df = pd.read_csv(data_info.path, sep=data_info.sep, names=data_info.header)
        if data_info.create:
            assert (
                data_info.rating_column_expression is not None
            ), "Cannot create rating column, no expression provided"
            assert data_info.raw_rating_column_name is not None, (
                "Cannot create rating column, no column name info " "provided"
            )
            df[data_info.raw_rating_column_name] = df[
                data_info.raw_rating_column_name
            ].apply(data_info.rating_column_expression)
        df = pd.pivot_table(
            df,
            values=data_info.pivot_info.values,
            index=[data_info.pivot_info.index],
            columns=[data_info.pivot_info.columns],
        )
        train_df, test_df = train_test_split(
            df.fillna(0), test_size=test_size, random_state=self.seed, shuffle=shuffle
        )
...
class Explorer:
    ...
    def experiment(
        self, bandits: List[BaseBandit], n_rounds: int
    ...
    def save_experiment(
        self,
        bandits: List[CascadeBanditBase],
    ...
    def to_dict(self) -> Dict[str, Union[str, float, int]]:
        return {"name": self.__class__.__name__, "data_info": self.helper.to_dict()}
    ...
    def save_winner_info(self, regret_stats: Dict[str, List[float]]):
        with open(f"{self.full_path}/experiment_results.txt", "a") as f:
            for bandit_name, regret_stats in regret_stats.items():
                f.write(
                    f"Mean regret for {bandit_name}:  {np.mean(regret_stats)}" + "\n"
                )
