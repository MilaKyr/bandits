...
class CascadeBanditBase(BaseBandit):
    def __init__(
        self, features: np.ndarray, variance: float, n_arms_to_show: int, max_items: int
    ):
...
class CascadeLinTS(CascadeBanditBase):
    ...
    def sample_theta(
        self, previous_theta, inverse_identity_feature_matrix
    ) -> np.ndarray:
        try:
            return np.random.multivariate_normal(
                previous_theta, inverse_identity_feature_matrix
            )
        except np.linalg.LinAlgError as e:
            return np.random.multivariate_normal(
                *self.initialize_vector_b_and_matrix_m()
            )
...
class CascadeLinUCB(CascadeBanditBase):
    ...
    def get_item_scores(self) -> List[float]:
        ...
        upper_confident_bound = (self.features.T @ inv_matrix_m * self.features.T).sum(
            1
        )
